...
<div class="flex flex-row grow max-w-full-1/2">
    <section class="p-1 grow max-w-full-1/2 ">
      <div class="flex flex-row grow max-w-full-1/2">
        <h4 class="flex flex-row text-xl font-bold text-white -mx-auto my-5">Home</h4>


      {#if mode.view}

        <button class="flex flex-row float-right justify-center p-0.5 mb-6 mr-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-blue-500 to-blue-300 group-hover:from-blue-300 group-hover:to-blue-500 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-blue-200 dark:focus:ring-blue-800"
                on:click={handleEdit}>
      ...
      {/if}
        ...
        <div class=" my-6 rounded-lg ">
            ...
            <div transition:blur|local="{{duration: 200}}" class="flex flex space-x-4 grow mr-4 rounded-lg">
                    <div transition:blur|local="{{duration: 200}}" class="flex-1 w-1/2 bg-white min-h-screen border-0 rounded-lg"
                         hidden="{mode.edit === true ? '' : 'hidden'}">
                        <div transition:blur|local="{{duration: 200}}" id="editor" class="editor bg-white"
                             hidden="{mode.edit === true ? '' : 'hidden'}"></div>
                    </div>
                  <div class="editor flex-1 w-1/2 bg-white p-2 min-h-screen border-0 rounded-lg"

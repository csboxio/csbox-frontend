<script lang="ts">
    ...
    let isOpen = localStorage.getItem(id + 'module_accordion') === 'true' || false;
    const isOpenStore = writable(isOpen);


    isOpenStore.subscribe(value => {
        localStorage.setItem(id + 'module_accordion', value.toString());
    });
    ...
    <button
        ...
        <div class="
            ...
            text-gray-300"
             ...
             class:text-blue-400={isHovered || isFocused}>
            {#if !published}
                <div class="hover:text-white" title="Not published"  on:click|stopPropagation={async () => { await publishModule() }}>
                    <Fa icon={faCircleXmark}></Fa>
                </div>
                {:else}
                <div class="hover:text-white" title="Published"  on:click|stopPropagation={async () => await unpublishModule()}>

<script lang="ts">
  ...
  let navItems = [
    { url: '/d/workspaces/view', admin: false, id: "workspaces", disabled: "false", edge: "true", svg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path d="M3 3H21C21.5523 3 22 3.44772 22 4V20C22 20.5523 21.5523 21 21 21H3C2.44772 21 2 20.5523 2 20V4C2 3.44772 2.44772 3 3 3ZM16.4645 15.5355L20 12L16.4645 8.46447L15.0503 9.87868L17.1716 12L15.0503 14.1213L16.4645 15.5355ZM6.82843 12L8.94975 9.87868L7.53553 8.46447L4 12L7.53553 15.5355L8.94975 14.1213L6.82843 12ZM11.2443 17L14.884 7H12.7557L9.11597 17H11.2443Z" fill="currentColor"></path></svg>'},
  ...
  if (claim == 'student') {
    navItems = navItems.filter(item => item.id !== 'org')
  }
...
<style>
  /* Custom style for darkening the image */
  .darken-white {
    filter: brightness(0.3); /* You can adjust the brightness value as needed */
  }
</style>
<div class="z-1 relative">
  <!-- Top mobile menu bar-->
  <nav class="lg:hidden flex items-center justify-between px-6 py-4 dark:bg-gray-700 bg-white mb-0.5">
    <div class="xl:hidden">
      <!-- Navbar-burger on the left -->
      <button class="navbar-burger text-gray-400 hover:text-gray-300 focus:outline-none" on:click={toggleMobileMenu}>
        <svg class="svelte-fa svelte-1cj2gr0" style="height:1em;line-height:.75em;font-size:1.33333em;vertical-align:-.225em;transform-origin:center;overflow:visible" viewBox="0 0 448 512" aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg">
          <g transform="translate(224 256)" transform-origin="112 0">
            <g transform="translate(0,0) scale(1,1)">
              <path d="M0 96C0 78.3 14.3 64 32 64H416c17.7 0 32 14.3 32 32s-14.3 32-32 32H32C14.3 128 0 113.7 0 96zM0 256c0-17.7 14.3-32 32-32H416c17.7 0 32 14.3 32 32s-14.3 32-32 32H32c-17.7 0-32-14.3-32-32zM448 416c0 17.7-14.3 32-32 32H32c-17.7 0-32-14.3-32-32s14.3-32 32-32H416c17.7 0 32 14.3 32 32z" fill="currentColor" transform="translate(-224 -256)"></path>
            </g>
          </g>
        </svg>
  ...
  {#if isMobileMenuOpen}
    <div transition:slide="{{  duration: 0,  axis: 'x' }}" class="fixed inset-0 bg-gray-700 opacity-75 z-40" on:click={toggleMobileMenu}></div>
    <nav transition:slide="{{   axis: 'x' }}" class="fixed inset-y-0 left-0  bg-white dark:bg-gray-800 shadow-lg z-50">
      <a class="inline-block mb-4 px-2" href={'#'}>
          ...
          {#each navItems as item}
            ...
            <li class:active={currentRoute === item.id} on:click={() => { currentRoute = item.id; }}>
              ...
              <a class="flex items-center justify-center w-12 h-12 rounded-xl dark:hover:bg-gray-800 hover:bg-gray-100 mb-2 "
                 ...
                 class:text-blue-400={currentRoute === item.id}
                 href={item.url}
                 sveltekit:prefetch
              >
                <div class="relative" >
      ...
      <!--pt-3 pb-3 px-8 dark:bg-gray-700 bg-white-->
      <div class="mt-auto">
        ...
        <a class="flex items-center justify-center w-12 h-12 rounded-xl text-gray-400 hover:text-blue-500 dark:hover:bg-gray-800 hover:bg-gray-100" href="/d/profile"
        ...
        <a class="flex items-center justify-center w-12 h-12 rounded-xl text-gray-400 hover:text-blue-500 dark:bg-gray-800 hover:bg-gray-100" href={'#'}>
  ...
  <div class="hidden lg:block navbar-menu relative z-10 ">
    <div class="lg:hidden navbar-backdrop fixed top-0 left-0 w-full h-full dark:bg-gray-800 bg-gray-100 opacity-50 "></div>
    <nav class="fixed top-0 left-0 bottom-0 max-w-max flex flex-col h-full py-6 px-2 dark:bg-gray-700 bg-white overflow-auto border-r border-gray-500">
      ...
      <div class="mt-auto">
        ...
        <a class="flex items-center justify-center w-12 h-12 rounded-xl text-gray-400 hover:text-blue-500 dark:hover:bg-gray-800 hover:bg-gray-200" href="/d/profile"
           on:click={() => currentRoute = "profile"}
           class:text-gray-400={currentRoute !== "profile"}
           class:text-blue-500={currentRoute === "profile"}>
          <svg width="20" height="22" viewbox="0 0 20 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        